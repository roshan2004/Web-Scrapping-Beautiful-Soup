...
html = requests.get('http://www.knockhardy.org.uk/ppoints.htm')
...
for link in soup.find_all('a', attrs = {'href': re.compile('.ppt$')}):
    links.append(link.get('href'))
   
#Since there are many links of the pdf files, I will loop over only first 10 links

links = sorted(links, key = lambda a: a.lower().split('/')[-1])
